...
export const createProcesoSchema = Joi.object({
  ...
  numeroIdentificacionCliente: Joi.string().pattern(/^\d{7,10}$/).required().messages({
    'string.pattern.base': 'El número de identificación del cliente debe tener entre 7 y 10 dígitos.',
    'any.required': 'El número de identificación del cliente es obligatorio.'
  }),
  
  numeroIdentificacionAbogado: Joi.string().pattern(/^\d{7,10}$/).required().messages({
    'string.pattern.base': 'El número de identificación del abogado debe tener entre 7 y 10 dígitos.',
