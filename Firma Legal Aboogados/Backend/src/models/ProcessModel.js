...
const procesoSchema = new mongoose.Schema({
  ...
  numeroIdentificacionCliente: {
    ...
    minlength: 7,
    maxlength: 10,
    match: [/^\d{7,10}$/, 'El número de identificación debe contener solo dígitos'],
    validate: {
      validator: async function (value) {
        const cliente = await Cliente.findOne({ numeroIdentificacion: value });
        return cliente != null;
      },
  ...
  numeroIdentificacionAbogado: {
    ...
    validate: {
      validator: async function (value) {
        const abogado = await Abogado.findOne({ numeroIdentificacion: value });
        return abogado != null;
      },
